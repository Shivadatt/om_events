(() => {
    ...
    $("#productGrid").innerHTML = state.items.map(item => `
      <article class="product-card">
        <div class="product-image">
          <img src="${escapeHtml(item.image_url)}" alt="${escapeHtml(item.name)} event setup" loading="lazy" width="600" height="480">
        ...
        <div class="product-info"><span class="product-meta">${escapeHtml(item.category)} · ${item.duration_hours} HRS</span><h3>${escapeHtml(item.name)}</h3>
          <p>${escapeHtml(item.description)}</p><div class="product-foot"><span class="price"><strong>${money(item.effective_price)}</strong>${item.offer_price ? `<s>${money(item.price)}</s>` : ""}<br>starting price</span><span class="rating">★ ${item.rating} (${item.review_count})</span></div>
        </div>
      </article>`).join("");
  ...
  function openDetail(slug) {
    ...
    if (!item) return;
    $("#detailContent").innerHTML = `<div class="detail-grid"><div class="detail-image"><img src="${escapeHtml(item.image_url)}" alt="${escapeHtml(item.name)}"></div>
      ...
      <div class="detail-price">${money(item.effective_price)} <small>starting</small></div>
  ...
  function updateCart() {
    ...
    const subtotal = state.cart.reduce((sum, row) => sum + row.product.effective_price * row.quantity, 0);
    const discount = subtotal >= 50000 ? subtotal * .05 : 0, taxable = subtotal - discount + (count ? 500 : 0), gst = taxable * .18;
    $("#subtotal").textContent = money(subtotal); $("#discount").textContent = discount ? `− ${money(discount)}` : "Unlock at ₹50k";
    $("#gst").textContent = money(gst); $("#grandTotal").textContent = money(taxable + gst);
  }
